...
#if !(defined(_WIN32) || defined(WINVER)) && !defined(__OS2__)
#include <unistd.h>
...
{
   pads_mutex_lock(&log_mutex);
}
static inline void unlock_logfile(void)
{
   pads_mutex_unlock(&log_mutex);
}
static inline void lock_loginit(void)
{
   pads_mutex_lock(&log_init_mutex);
}
static inline void unlock_loginit(void)
{
   pads_mutex_unlock(&log_init_mutex);
}
 ...
 *                a WIN32 GUI, to a dialog box), and exits pAds
 *                with status code 1.
 *
 * Parameters  :
...
static void fatal_error(const char *error_message)
{
   ...
   MessageBox(g_hwndLogFrame, box_message, "pAds Error", 
 ...
 *
 * Description :  Logs the pAds version and the program name.
 *
 * Parameters  :
...
{
   log_error(LOG_LEVEL_INFO, "pAds version " VERSION);
   if (prog_name != NULL)
   {
...
{
   debug = debug_level | LOG_LEVEL_MINIMUM | LOG_LEVEL_FORCE;
}
...
void init_error_log(const char *prog_name, const char *logfname)
{
   ...
   if ((NULL == fp) && (logfp != NULL))
   {
      ...
      log_error(LOG_LEVEL_INFO, "Failed to reopen logfile: \'%s\'. "
         ...
         "doesn't work, pAds will exit without being able to log a message.",
         logfname);
      lock_logfile();
      fclose(logfp);
      logfp = NULL;
