 ...
 *
 *********************************************************************/
jb_err simple_read_line(FILE *fp, char **dest, int *newline, const char* fname)
{
	...
	if (fp == NULL) // if file not exist.
	{
		...
		return utl_file_line(fp, dest, newline, fname);
	}
#endif

   if (!fp) return JB_ERR_FILE;
					  ...
					  const char* fname)
{
   ...
   while ( (contflag || is_empty)
        && (JB_ERR_OK == (rval = simple_read_line(fp, &linebuf, newline, fname))))
   {
      if (line_number)
      {
         (*line_number)++;
 ...
 *
 *********************************************************************/
char *read_config_line(char *buf, size_t buflen, FILE *fp, unsigned long *linenum, const char* fname)
{
   jb_err err;
   char *buf2 = NULL;
   err = edit_read_line(fp, NULL, NULL, &buf2, NULL, linenum, fname);
   if (err)
   {
...
int load_trustfile(struct client_state *csp)
{
   ...
   while (read_config_line(buf, sizeof(buf), fp, &linenum, csp->config->trustfile) != NULL)
   {
      trusted = 0;
      reject  = 1;
...
int load_one_re_filterfile(struct client_state *csp, int fileid)
{
    ...
    */
   fp = fopen(csp->config->re_filterfile[fileid], "r");
#if 0
   if (fp == NULL)
   {
      goto load_re_filterfile_error;
    ...
    * Read line by line
    */
   while (read_config_line(buf, sizeof(buf), fp, &linenum, csp->config->re_filterfile[fileid]) != NULL)
   {
