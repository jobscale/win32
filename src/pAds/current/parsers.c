   ...
   if (strstr(*header, "gzip") || strstr(*header, "compress") || strstr(*header, "deflate"))
   {
         ...
         (!list_is_empty(csp->action->multi[ACTION_MULTI_FILTER])))
      {
         log_error(LOG_LEVEL_INFO,
            ...
            "Consider recompiling pAds with zlib support or "
 ...
 *                of the document has been altered by pAds, the server
      ...
      while (*cur_tag)
      {
         ...
         if ((strncmpic(cur_tag, "expires=", 8) == 0) && *(cur_tag + 8))
         {
            ...
            else
            {
                ...
                *   been done until pAds 3.0.6 and to live with
