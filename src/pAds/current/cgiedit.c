...
struct editable_file
{
   ...
   const char * filename;     /**< Full pathname - e.g. "/etc/pads/wibble.action". */
...
static jb_err edit_read_file_lines(FILE *fp, struct file_line ** pfile, int *newline, const char* fname);
 ...
 *
 *********************************************************************/
jb_err edit_read_file_lines(FILE *fp, struct file_line ** pfile, int *newline, const char* fname)
{
   ...
   *pfile = NULL;
   ...
   rval = edit_read_line(fp, &cur_line->raw, &cur_line->prefix, &cur_line->unprocessed, newline, NULL, fname);
   if (rval)
   {
                      ...
                      struct editable_file **pfile)
{
   ...
   *pfile = NULL;
   ...
   else if (JB_ERR_CGI_PARAMS == err)
   {
       ...
       * http://config.pads.com/edit-actions-list?f=default
       */
   ...
   err = edit_read_file_lines(fp, &lines, &newline, filename);
 ...
 *                accepts files that pAds already knows.
 *
 * Parameters  :
 *          1  :  csp = Current client state (buffers, headers, etc...)
 *          2  :  parameters = map of cgi parameters
                                ...
                                const struct map *parameters)
{
    ...
    *
    * In pAds 3.0.5 beta the standard edit-actions-for-url template
