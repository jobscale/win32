...
//   }
   ...
   g_hwndLogFrame = CreateLogWindow(g_hInstance, /*g_nCmdShow*/SW_HIDE);
   g_hwndTray = CreateTrayWindow(g_hInstance);
   // TrayAddIcon(g_hwndTray, 1, g_hiconApp, "Log Ads");
...
{
   static const char *szWndName = "pAdsLogOwner";
   WNDCLASS wc;
   HWND hwnd;
...
{
   static const char *szWndName = "pAdsLogWindow";
   static const char *szWndTitle = "pAds";
...
//      case ID_HELP_ABOUT:
//         MessageBox(g_hwndLogFrame, win32_blurb, "About pAds", MB_OK);
//         break;
...
void SetIdleIcon()
{
         ...
         if (!global_toggle_state)
         {
            ...
            /* log_error(LOG_LEVEL_INFO, "pAds OFF icon selected."); */
         }
         else
#endif /* def FEATURE_TOGGLE */
