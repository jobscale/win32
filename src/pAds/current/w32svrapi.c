const char w32_svrapi_rcs[] = "";
/*********************************************************************
 *
 * File        :  $Source: /cvsroot/ijbswa/current/w32svrapi.c,v $
 *
 * Purpose     :  Win32 Services API for pAds.
 ...
 */
#ifdef UNICODE
#error "pAds interface to Win32 Services only runs under ANSI builds. Unicode is not supported at present, but you can volunteer for the job if you like! :)"
#endif
...
static void WINAPI pads_w32_service_start(DWORD dw, LPSTR* psz);
static void WINAPI pads_w32_service_handler(DWORD dwOpcode);
SERVICE_TABLE_ENTRY w32ServiceDispatchTable[] = {{"", pads_w32_service_start}, {NULL, NULL}};
...
static void display_win32_msg(BOOL bIsError, char *msg)
{
   ...
   if (bIsError)
   {
      MessageBox(NULL, msg, "pAds Error", 
         MB_OK | MB_ICONERROR | MB_TASKMODAL | MB_SETFOREGROUND | MB_TOPMOST);
   }
   else
   {
      MessageBox(NULL, msg, "pAds Information", 
...
BOOL install_service(const char *service_name)
{
   ...
   {
      display_win32_msg(TRUE, "This system doesn't support installing pAds as a service.\nWinNT/2000/XP are required for this feature.\n");
      return FALSE;
   }
   ...
   if (memcmp(service_name, "plads", strlen("plads") + 1) == 0)
   {
      sprintf(szModule, "\"%s\" --service %s", szDisplayName, "pad-config-pl.txt"); 
   }
...
#endif
                           ,
//                           SERVICE_DEMAND_START,      /* For now, only start when asked to SERVICE_AUTO_START */
                           SERVICE_AUTO_START,      /* For now, only start when asked to SERVICE_DEMAND_START */
...
/*********************************************************************
 *
 * Function    :  pads_w32_service_start
 *
 ...
 *
 *********************************************************************/
static void WINAPI pads_w32_service_start(DWORD dw, LPSTR* pszArgs)
{
   int child_id;
   ...
   hSrv_status = w32_register_service_ctrl_handler(szThisServiceName, pads_w32_service_handler);
   if (!hSrv_status)
   {
      return;
   }
...
/*********************************************************************
 *
 * Function    :  pads_w32_service_handler
 *
