...
const char *pcrs_strerror(const int error)
{
   if (error < 0)
   {
      switch (error)
      {
          ...
          * version. If pAds is linked against a newer
          * PCRE version all bets are off ...
          */
         default:  return "Unknown error. pAds out of sync with PCRE?";
      }
   }
   /* error >= 0: No error */
 ...
 * included in pAds. If you use any of them you should not
