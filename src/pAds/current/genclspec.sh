...
VERSION=`cat pads-rh.spec | sed -n -e 's/^Version:[ ]*//p'`
RELEASE=`cat pads-rh.spec | sed -n -e 's/^Release:[ ]*//p'`
CLTAG=${VERSION}-${RELEASE}cl

PACKAGER=`rpm --eval "%{packager}"`
if [ "${PACKAGER}" = "%{packager}" ]; then
	PACKAGER="genclspec script <developers@pads.com>"
fi
...
if [ -r pads-cl.spec ]; then
	echo Old CL specfile found. Removing it.
fi

cat pads-rh.spec | sed -e 's/^\(Release:[ ]*[^ ]\+\)[ ]*$/\1cl/' \
			  -e "/^%changelog/a* ${DATETAG} ${PACKAGER}" \
			  -e "/^%changelog/a+ pads-${CLTAG}" \
