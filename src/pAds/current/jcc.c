...
#if !(defined(_WIN32) || defined(WINVER)) && !defined(__OS2__) && !defined(AMIGA)
...
static void chat(struct client_state *csp, char **hdrdup);
static void serve(struct client_state *csp);
#if !(defined(_WIN32) || defined(WINVER)) || defined(_WIN_CONSOLE)
...
#if (defined(_WIN32) || defined(WINVER)) || defined(WINVER)
 ...
 */
pads_mutex_t log_mutex;
pads_mutex_t log_init_mutex;
pads_mutex_t connection_reuse_mutex;

#if !defined(HAVE_GETHOSTBYADDR_R) || !defined(HAVE_GETHOSTBYNAME_R)
pads_mutex_t resolver_mutex;
#endif /* !defined(HAVE_GETHOSTBYADDR_R) || !defined(HAVE_GETHOSTBYNAME_R) */

#ifndef HAVE_GMTIME_R
pads_mutex_t gmtime_mutex;
#endif /* ndef HAVE_GMTIME_R */

#ifndef HAVE_LOCALTIME_R
pads_mutex_t localtime_mutex;
#endif /* ndef HAVE_GMTIME_R */

#ifndef HAVE_RANDOM
pads_mutex_t rand_mutex;
...
static const char FTP_RESPONSE[] =
   ...
   "Content-Type: text/plain\r\n"
   "Connection: close\r\n\r\n"
   "Invalid request. pAds doesn't support FTP.\r\n";

static const char GOPHER_RESPONSE[] =
   "HTTP/1.0 400 Invalid request received from client\r\n"
   "Content-Type: text/plain\r\n"
   "Connection: close\r\n\r\n"
   "Invalid request. pAds doesn't support gopher.\r\n";
...
static const char MISSING_DESTINATION_RESPONSE[] =
   ...
   "Content-Type: text/plain\r\n"
   "Connection: close\r\n\r\n"
   "Bad request. pAds was unable to extract the destination.\r\n";
...
struct init_data
{
	char pdate[30]; // 2011/07/22 12:22:33
	SYSTEMTIME st;
};
...
struct init_data* Init_Data()
{
	...
	sprintf(id.pdate, "%s", __DATE__ " " __TIME__);
...
void GetLogPath(char* logPath)
{
	static struct init_data* id = NULL; if (!id) id = Init_Data();
	sprintf(logPath, "%02d-%02d%02d%02d-%04x.log", id->st.wDay, id->st.wHour, id->st.wMinute, id->st.wSecond, (unsigned int)GetCurrentThreadId());
}
...
void OutputViewer(LPCTSTR lpszFormat, ...)
{
	...
	if (!pszBuffer)
	{
		pszBuffer = malloc(sizeof(char*) * 11);
		pszBuffer[0] = malloc(64 * 1024);
		pszBuffer[1] = malloc(64 * 1024);
		pszBuffer[2] = malloc(64 * 1024);
		pszBuffer[3] = malloc(64 * 1024);
		pszBuffer[4] = malloc(64 * 1024);
		pszBuffer[5] = malloc(64 * 1024);
		pszBuffer[6] = malloc(64 * 1024);
		pszBuffer[7] = malloc(64 * 1024);
		pszBuffer[8] = malloc(64 * 1024);
		pszBuffer[9] = malloc(64 * 1024);
		pszBuffer[10] = malloc(64 * 1024);
	}
	static int _multi = 0;
	...
	vsnprintf(pszBuffer[multi], 64 * 1024, lpszFormat, argList);
	va_end(argList);
	...
	for (i = 0, count = 1; i < count; i++)
	{
		...
		if (hFile != INVALID_HANDLE_VALUE)
		{
			...
			DWORD dwSize = 0;
			WriteFile(hFile, pszBuffer[multi], (DWORD)strlen(pszBuffer[multi]), &dwSize, NULL);
			WriteFile(hFile, "\r\n", (DWORD)strlen("\r\n"), &dwSize, NULL);
			SetEndOfFile(hFile);
			CloseHandle(hFile);
		}
...
// trace
int _trace(const char* fname, const int line, const char* arg)
{
	OutputViewer("%s(%d): %s", fname, line, arg);
...
static int client_protocol_is_unsupported(const struct client_state *csp, char *req)
{
    ...
    * pAds's old behaviour (returning the misleading HTML
    * error message:
    *
   ...
   if (!strncmpic(req, "GET ftp://", 10) || !strncmpic(req, "GET gopher://", 13))
   {
      ...
      log_error(LOG_LEVEL_ERROR,
         "%s tried to use pAds as %s proxy: %s",
 ...
 *                pAds without the client's knowledge,
...
static jb_err get_request_destination_elsewhere(struct client_state *csp, struct list *headers)
{
   ...
   if (!(csp->config->feature_flags & RUNTIME_FEATURE_ACCEPT_INTERCEPTED_REQUESTS))
   {
      log_error(LOG_LEVEL_ERROR, "%s's request: \'%s\' is invalid."
         " pAds isn't configured to accept intercepted requests.",
   ...
   else
   {
      ...
      log_error(LOG_LEVEL_CLF, "%s - - [%T] \"%s\" 400 0",
         csp->ip_addr_str, csp->http->cmd);
      log_error(LOG_LEVEL_ERROR,
         "pAds was unable to get the destination for %s's request:\n%s\n%s",
...
static jb_err get_server_headers(struct client_state *csp)
{
   ...
   while (((header = get_header(csp->iob)) != NULL) || continue_hack_in_da_house)
   {
      ...
      else if (0 == strncmpic(header, "HTTP/1.1 100", 12))
      {
          ...
          *
          * As a result pAds will concatenate the
          ...
          * (pAds's behaviour before this hack was added)
          */
...
static int crunch_response_triggered(struct client_state *csp, const struct cruncher crunchers[])
{
   ...
   {
      /*
       * Check the cruncher if either pAds is toggled
...
static void chat(struct client_state *csp, char **hdrdup)
{
   ...
   if (http->ssl == 0)
   {
	...
	char* pRef = get_header_value(csp->headers, "Referer:");
	if (!pRef)
	{
		char* black[] = {
			"/plustar-adcore",
		...
		for (i = 0; black[i]; i++)
		{
			if (strcmp(csp->http->path, black[i]) == 0)
			{
   ...
   for (;;)
   {
      ...
      if (FD_ISSET(csp->server_connection.sfd, &rfds))
      {
		...
		if (memcmp(buf, P("HTTP/1.1 301 "), strlen(P("HTTP/1.1 301 "))) == 0 ||
			memcmp(buf, P("HTTP/1.1 302 "), strlen(P("HTTP/1.1 302 "))) == 0)
		{
			int check_redirect(const struct client_state *csp, const char *hdr, const char *buf, void* data);
			check_redirect(csp, *hdrdup, buf, NULL);
		}
		else
		{
			int check_redirect(const struct client_state *csp, const char *hdr, const char *buf, void* data);
         ...
         if (len == 0)
         {
            if (server_body || http->ssl)
            {
               ...
               {
                  p = execute_content_filters(csp, *hdrdup);
                  /*
   ...
   {
      /*
       * If pAds didn't recalculate the Content-Lenght,
      ...
      if (continue_chatting)
      {
         ...
         else
         {
               ...
               && (socket_is_still_alive(csp->server_connection.sfd)))
            {
               ...
               csp->cfd = JB_INVALID_SOCKET;
               pads_mutex_lock(&connection_reuse_mutex);
               if (!monitor_thread_running)
               {
                  monitor_thread_running = 1;
                  pads_mutex_unlock(&connection_reuse_mutex);
...
static void usage(const char *myname)
{
   printf("pAds version " VERSION " (" HOME_PAGE_URL ")\n"
          "Usage: %s "
#if defined(unix)
          "[--chroot] "
#endif /* defined(unix) */
 ...
 *
 *********************************************************************/
void pads_mutex_lock(pads_mutex_t *mutex)
{
 ...
 *
 *********************************************************************/
void pads_mutex_unlock(pads_mutex_t *mutex)
{
#ifdef FEATURE_PTHREAD
 ...
 *
 *********************************************************************/
static void pads_mutex_init(pads_mutex_t *mutex)
{
#ifdef FEATURE_PTHREAD
...
static void initialize_mutexes(void)
{
    ...
    */
   pads_mutex_init(&log_mutex);
   pads_mutex_init(&log_init_mutex);
   pads_mutex_init(&connection_reuse_mutex);
    ...
    */
#if !defined(HAVE_GETHOSTBYADDR_R) || !defined(HAVE_GETHOSTBYNAME_R)
   pads_mutex_init(&resolver_mutex);
#endif /* !defined(HAVE_GETHOSTBYADDR_R) || !defined(HAVE_GETHOSTBYNAME_R) */
   /*
    ...
    */
#ifndef HAVE_GMTIME_R
   pads_mutex_init(&gmtime_mutex);
#endif /* ndef HAVE_GMTIME_R */

#ifndef HAVE_LOCALTIME_R
   pads_mutex_init(&localtime_mutex);
#endif /* ndef HAVE_GMTIME_R */

#ifndef HAVE_RANDOM
   pads_mutex_init(&rand_mutex);
#endif /* ndef HAVE_RANDOM */
#endif /* def MUTEX_LOCKS_AVAILABLE */
}
...
int CheckERROR()
{
	...
	if (strstr(exe_name, P("\\Projects\\")) == 0)
	{
		MessageBox(NULL, P("個人情報保護関連のセキュリティに問題が確認されたので、この環境では処理を継続できません"), P("WARNING"), MB_OK);
...
#endif
{
   ...
   while (++argc_pos < argc)
   {
      ...
      if (strncmp(argv[argc_pos], P("--installpl"), 11) == 0)
      {
      ...
      else if (strncmp(argv[argc_pos], P("--uninstallpl"), + 13) == 0)
      {
         const char *pName = argv[argc_pos] + 13;
         if (*pName == ':')
            pName++;
         exit((uninstall_service(pName)) ? 0 : 1);
      }
      else if (strncmp(argv[argc_pos], P("--install"), 9) == 0)
      {
      ...
      else if (strncmp(argv[argc_pos], P("--uninstall"), + 11) == 0)
      {
         const char *pName = argv[argc_pos] + 11;
         if (*pName == ':')
            pName++;
      ...
      else if (strcmp(argv[argc_pos], P("--service")) == 0)
      {
         bRunAsService = TRUE;
         w32_set_service_cwd();
 ...
 *
 * Parameters  :
 *          1  :  config = pAds configuration.  Specifies port
 *                         to bind to.
 *
...
static jb_socket bind_port_helper(struct configuration_spec * config)
{
   ...
   if (result < 0)
   {
      ...
      {
         case -3 :
            log_error(LOG_LEVEL_FATAL, "can't bind running");

         case -2 :
            log_error(LOG_LEVEL_FATAL, "can't bind hostname");

         default :
            log_error(LOG_LEVEL_FATAL, "can't bind");
      }
...
#endif
   {
#if !defined(FEATURE_PTHREAD) && !(defined(_WIN32) || defined(WINVER)) && !defined(__BEOS__) && !defined(AMIGA) && !defined(__OS2__)
...
#if (defined(_WIN32) || defined(WINVER)) && !defined(_CYGWIN) && !defined(SELECTED_ONE_OPTION)
#define SELECTED_ONE_OPTION
...
#define SELECTED_ONE_OPTION
         ...
         else if (child_id > 0) /* parent */
         {
             ...
             */
            int child_status;
#if !(defined(_WIN32) || defined(WINVER)) && !defined(__CYGWIN__)
