...
jb_socket connect_to(const char *host, int portnum, struct client_state *csp)
{
   ...
   fd_set wfds;
   struct timeval tv[1];
#if !(defined(_WIN32) || defined(WINVER)) && !defined(__BEOS__) && !defined(AMIGA) && !defined(__OS2__)
   int   flags;
#endif
   int connect_failed;
   ...
   for (rp = result; rp != NULL; rp = rp->ai_next)
   {
         ...
         && FD_ISSET(fd, &wfds))
      {
         ...
         if (!getsockopt(fd, SOL_SOCKET, SO_ERROR, (char*)&socket_in_error, &optlen))
         {
            if (!socket_in_error)
            {
...
{
#if (defined(_WIN32) || defined(WINVER)) || defined(__BEOS__)
   closesocket(fd);
#elif defined(AMIGA)
   CloseSocket(fd); 
#elif defined(__OS2__)
...
int bind_port(const char *hostnam, int portnum, jb_socket *pfd)
{
   ...
   *pfd = JB_INVALID_SOCKET;
    ...
    * duplicate instances of pAds from being caught.
    *
    * On UNIX, we assume the user is sensible enough not
    * to start pAds multiple times on the same IP.
    * Without this, stopping and restarting pAds
    * from a script fails.
...
void get_host_information(jb_socket afd, char **ip_address, char **hostname)
{
  ...
  memset(&adapter_addrs, 0, len);

  ULONG family = AF_INET;
  ULONG flags = 0x0010; //GAA_FLAG_INCLUDE_PREFIX;

  if (GetAdaptersAddresses(family, flags, NULL, &adapter_addrs, &len) == ERROR_SUCCESS)
  {
  }
#endif
...
#endif /* HAVE_RFC2553 */
#if (defined(_WIN32) || defined(WINVER)) || defined(__OS2__) || defined(__APPLE_CC__) || defined(AMIGA)
   ...
   *ip_address = NULL;
   ...
   if (!getsockname(afd, (struct sockaddr *) &server, &s_length))
   {
      ...
      *hostname = malloc(NI_MAXHOST);
      ...
      pads_mutex_lock(&resolver_mutex);
      host = gethostbyaddr((const char *)&server.sin_addr, 
                           sizeof(server.sin_addr), AF_INET);
      pads_mutex_unlock(&resolver_mutex);
#else
      host = gethostbyaddr((const char *)&server.sin_addr, 
                           sizeof(server.sin_addr), AF_INET);
#endif
