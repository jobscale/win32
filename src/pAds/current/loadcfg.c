...
struct configuration_spec * load_config(void)
{
   ...
   while (read_config_line(buf, sizeof(buf), configfp, &linenum, configfile) != NULL)
   {
 ...
 *   pAds returns a 200/OK status instead of a 403/Forbidden status
...
#if defined(_WIN_CONSOLE) || ! (defined(_WIN32) || defined(WINVER))
         case hash_activity_animation :
         case hash_close_button_minimizes :
         case hash_log_buffer_size :
         case hash_log_font_name :
